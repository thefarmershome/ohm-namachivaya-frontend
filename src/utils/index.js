...
function totalPrice(items, showRSSymbol = true) {
  const totalPrice = items.reduce((itemAcc, item) => {
    return (itemAcc += item.product.price * item.quantity);
  }, 0);
  return priceConversion(totalPrice, showRSSymbol);
}

const priceConversion = (price, showRSSymbol = true) => {
  if (showRSSymbol) {
